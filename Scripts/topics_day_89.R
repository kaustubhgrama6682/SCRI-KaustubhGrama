...
seuobj89 <- readRDS(file = "/Users/kaustubhgrama/Downloads/SCRI/seuobj89.RDS")

day89df <- as_data_frame(seuobj89@meta.data)

topic_cols_89 <- grep("^Topic_", names(day89df), value = TRUE)
...
day94df <-  as_data_frame(seuobj89@meta.data)

topic_cols_94 <- grep("^Day94_Topic_", names(day89df), value = TRUE)
...
day110df <-  as_data_frame(seuobj89@meta.data)

topic_cols_110 <- grep("^Day110_Topic_", names(day89df), value = TRUE)
...
day115df <-  as_data_frame(seuobj89@meta.data)

topic_cols_115 <- grep("^Day115_Topic_", names(day89df), value = TRUE)
...
day125df <-  as_data_frame(seuobj89@meta.data)

topic_cols_125 <- grep("^Day125_Topic_", names(day89df), value = TRUE)
...
seuobj89@meta.data$highest_topic <- day89df$highest_topic
seuobj89@meta.data$second_highest_topic <- day89df$second_highest_topic
seuobj89@meta.data$third_highest_topic <- day89df$third_highest_topic

# Subset to "Granule Neurons"
astrocyte_obj <- subset(seuobj89, subset = Main_cluster_name == "Astrocytes")
