...
for(gene in colnames(day115_125_gene_expression_data)[colnames(day115_125_gene_expression_data) != "Main_cluster_name"]){
  ...
  for(n in unique(count_df$count)){
    ...
    n_unipolar_brush_cells <- sum(day115_125_gene_expression_data[day115_125_gene_expression_data$Main_cluster_name == "Unipolar brush cells",gene]==n)
    
    # probability of a cell being an astrocyte
    # number of astrocytes w unique count value for spp1 / all cells
    percent <- n_unipolar_brush_cells/ sum(day115_125_gene_expression_data[,gene]==n)
...
training.data.topgenes$celltype[training.data.topgenes$Main_cluster_name == "Unipolar brush cells"] <- 0

testing.data.topgenes$celltype <- 1
testing.data.topgenes$celltype[testing.data.topgenes$Main_cluster_name == "Unipolar brush cells"] <- 0
...
day110_gene_expression_data$celltype[day110_gene_expression_data$Main_cluster_name == "Unipolar brush cells"] <- 0
...
undersample_odds <- sum(training.data$Main_cluster_name == "Unipolar brush cells")/sum(training.data$Main_cluster_name != "Unipolar brush cells")
...
seuobj110$predicted_unipolar_brush_cells_temp <- predicted.classes

seuobj110$probabilities_neg <- 1-probabilities

DimPlot(seuobj110, group.by = "predicted_unipolar_brush_cells_temp")
...
saveRDS(step.model.both, "/Users/kaustubhgrama/Desktop/Computer_Science/R/Data/fetal_cerebellar_scData/models/step.model.both_unipolar_brush_cells.RDS")
